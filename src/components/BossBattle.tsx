...
interface Message {import React, { useState, useRef, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { useGameState } from './GameStateContext';
import { Trophy, ShieldAlert, Cpu, Send, Bot, User, Loader2, ArrowLeft, Zap } from 'lucide-react';
...
// ─── KEYWORD FALLBACK ──────────────────────────────────────────────────────
// Fuzzy match: normalises user text so "6000 dollars", "6,000$", bare numbers,
// and single-word answers like "byoc" or "vpc" all register correctly.
function scoreKeyword(
    ...
    // Synonym substitution so natural language maps to keyword list
    const normalised = lower
        .replace(/\bdollars?\b/g, '$')
        .replace(/\bper\s+month\b/g, 'monthly')
        .replace(/\bcheap(er)?\b/g, 'cost')
        .replace(/\bpric(e|ing)\b/g, 'cost')
        .replace(/\bno\s+fee[s]?\b/g, 'no per-message')
        .replace(/\bour\s+(cloud|infra(structure)?|network)\b/g, 'vpc')
        .replace(/\b(bring\s+your\s+own\s+cloud|own\s+vpc|own\s+cloud)\b/g, 'byoc')
        .replace(/\b(support\s+team|on[\s-]call|dedicated\s+support)\b/g, 'sla')
        .replace(/\b(auto[\s-]?scal(e|ing)|scales?\s+automatically)\b/g, 'auto-scale')
        .replace(/\bno\s+(zookeeper|zoo\s*keeper)\b/g, 'no zookeeper')
        .replace(/\b(code[\s-]?defined|pipelines?\s+in\s+code)\b/g, 'pipeline');

    // Pure number answer (e.g. "6000") counts as a cost hit
    const isJustNumber = /^\d[\d\s,.]*(%|k|m|tb|gb)?$/.test(lower);

    for (const kw of persona.keywords) {
        const kwNorm = kw.toLowerCase();
        if (!newUsed.has(kw) && (normalised.includes(kwNorm) || lower.includes(kwNorm))) {
            newUsed.add(kw);
            return { hit: true, newUsed };
        }
    }

    if (isJustNumber && !newUsed.has('cost')) {
        newUsed.add('cost');
        return { hit: true, newUsed };
    }
...
export const BossBattle: React.FC<BossBattleProps> = ({ onComplete, onBack }) => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        // If still ooc after keyword check, use a hardcoded redirect so the bot
        // never goes silent on greetings like "hi" or "bye"
        if (evalResult.ooc) {
            const oocReplies = [
                `Let's keep this professional. ${selectedPersona.stageContext[Math.min(score, selectedPersona.scoreToWin - 1)].replace('Current concern: ', 'My question was: ').split(' Resolved')[0]}`,
                `I have limited time. Please address my question directly.`,
                `That is not relevant. Let's stay on topic.`,
            ];
            const oocReply = oocReplies[Math.min(newOocCount - 1, oocReplies.length - 1)];
            setIsTyping(false);
            setMessages(prev => [...prev, { id: `bot-ooc-${Date.now()}`, role: 'model' as const, content: oocReply }]);
