            ...
            <div className="max-w-6xl mx-auto relative z-10">
                <header className="flex justify-between items-center mb-10">
                    <div>
                        ...
                        <p className="text-gray-500">Global performance tracking across all active deployed agents.</p>
                    </div>

                    <button
                        onClick={fetchUsers}
                        disabled={loading}
                        className="flex items-center gap-2 bg-[#1e293b] hover:bg-[#334155] border border-gray-200 text-sm px-4 py-2 rounded-lg transition-colors"
                    >
                ...
                {error ? (
                    ...
                    >
                        <div className="flex bg-[#1e293b] p-4 gap-4 border-b border-gray-200 text-sm font-medium">
                            <button
                                onClick={() => setSortField('xp')}
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-md transition-colors ${sortField === 'xp' ? 'bg-sky-500/20 text-sky-400' : 'text-gray-500 hover:bg-gray-200'}`}
                            >
                                <Trophy className="w-4 h-4" /> Sort by Experience
                            </button>
                            <button
                                onClick={() => setSortField('accuracy')}
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-md transition-colors ${sortField === 'accuracy' ? 'bg-emerald-500/20 text-emerald-400' : 'text-gray-500 hover:bg-gray-200'}`}
                            >
                                <Target className="w-4 h-4" /> Sort by Accuracy
                            </button>
                            <button
                                onClick={() => setSortField('modulesCompleted')}
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-md transition-colors ${sortField === 'modulesCompleted' ? 'bg-purple-500/20 text-purple-400' : 'text-gray-500 hover:bg-gray-200'}`}
                            >
                                    ...
                                    {loading ? (
                                        ...
                                        users.map((user, index) => (
                                            <motion.tr
                                                ...
                                                key={user.id}
                                                className="border-b border-gray-200/50 hover:bg-[#1e293b]/50 transition-colors"
                                            >
                                                <td className="p-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-sky-400 font-bold text-xs ring-1 ring-slate-700">
                                                ...
                                                <td className="p-4">
                                                    <span className="inline-flex items-center px-2 py-1 rounded bg-gray-200/50 text-gray-700 text-xs border border-gray-200/50">
                                                        {user.companyCode}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="p-4 text-center">
                                                    <div className="flex items-center justify-center gap-2">
                                                        <span className="text-gray-700 font-medium">{user.modulesCompleted}</span>
                                                ...
                                                <td className="p-4 text-center">
                                                    <button
                                                        ...
                                                        className={`p-2 rounded-lg transition-colors ${expandedUser === user.id ? 'bg-sky-500 text-gray-900' : 'bg-gray-200 text-gray-500 hover:bg-gray-300'}`}
                ...
                <AnimatePresence>
                    {expandedUser && users.find(u => u.id === expandedUser) && (
                        ...
                        >
                            <div className="bg-white border border-gray-200 w-full max-w-4xl max-h-[80vh] rounded-[2rem] overflow-hidden flex flex-col shadow-2xl">
                                <header className="p-6 border-b border-gray-200 flex justify-between items-center bg-[#1e293b]">
                                    <div>
                                        <h2 className="text-xl font-bold flex items-center gap-2">
                                            Chat History: <span className="text-sky-400">{users.find(u => u.id === expandedUser)?.username}</span>
                                        </h2>
                                        <p className="text-gray-500 text-xs uppercase tracking-widest font-bold mt-1">AI Simulation Transcripts</p>
                                    </div>
                                    <button
                                        ...
                                        Object.entries(users.find(u => u.id === expandedUser)?.chatTranscripts || {}).map(([personaId, transcript]) => (
                                            <div key={personaId} className="space-y-4">
                                                <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <span className="text-[10px] text-gray-400 font-mono">{new Date(transcript.timestamp).toLocaleString()}</span>
                                                ...
                                                <div className="space-y-4 px-4 border-l-2 border-gray-200 ml-2">
                                                    {transcript.messages.map((msg, i) => (
                                                        <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-start' : 'items-end'}`}>
                                                            ...
                                                            <div className={`p-3 rounded-xl text-sm max-w-[90%] ${msg.role === 'user' ? 'bg-gray-200 text-gray-800 rounded-tl-none' : 'bg-sky-900/40 text-sky-100 rounded-tr-none border border-sky-500/20'}`}>
                                                                {msg.content}
                                                            </div>
                                                        </div>
                                                    ))}
